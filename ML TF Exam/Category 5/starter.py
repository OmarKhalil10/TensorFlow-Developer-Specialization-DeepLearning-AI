...
def solution_model():
    ...
    model = tf.keras.models.Sequential([
        ...
        # since it has not been implemented in the cuDNN kernel and may
        # result in much longer training times.
        tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(12, return_sequences=True, input_shape=[BATCH_SIZE, N_PAST, N_FEATURES])),
    ...
    model_checkpoint = tf.keras.callbacks.ModelCheckpoint("my_check.h5", save_best_only=True)
    model.fit(
        # YOUR CODE HERE
        train_set,
        epochs=100,
    ...
    model= tf.keras.models.load_model("my_check.h5")
